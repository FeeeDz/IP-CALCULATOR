...
//function that finds the bits dedicated to subnetting in the IP address
...
void banner(int mod)
{
    ...
    char ban2[] = "8b           d8 88          ad88888ba  88b           d88  \n"
                  "`8b         d8' 88         d8      8b  888b         d888  \n"
                  " `8b       d8'  88         Y8,         88`8b       d8'88  \n"
                  "  `8b     d8'   88         `Y8aaaaa,   88 `8b     d8' 88  \n"
                  "   `8b   d8'    88            888888b, 88  '8b   d8'  88  \n"
                  "    `8b d8'     88                 `8b 88   `8b d8'   88  \n"
                  "     `888'      88         Y8a     a8P 88    '888'    88  \n"
                  "      `8'       88888888888  Y88888P   88     `8'     88   \n";
    char ban3[] =
        "                      88                                           \n"
        "                      88                                   ,d      \n"
        "                      88                                   88      \n"
        ",adPPYba, 88       88 88,dPPYba,  8b,dPPYba,   ,adPPYba, MM88MMM   \n"
        "I8[       88       88 88P      8a 88P      8a a8P_____88   88      \n"
        "   Y8ba,  88       88 88       d8 88       88 8888888888   88      \n"
        "aa    ]8I  8a,   ,a88 88b,   ,a8  88       88  8b,   ,aa   88,     \n"
        "  YbbdP      YbbdP Y8 8Y Ybbd8    88       88    Ybbd8     Y888   \n";
...
//data printing and processing SUBNETTING
/*
the cycles for add "block" to the octet dedicated to the hosts 
in class A the second, third, fourth
in class B the third and the fourth
in class C the fourth
*/
void print_ip(int ip1, int ip2, int ip3, int ip4, int block, int nsubnet)
{
    ip4 = 0;
    fp = fopen("ip.txt", "a");
    fprintf(fp, "SUBNETTING\n");
    switch (classe(ip1))
    {
    case 'A':
        fprintf(fp, "\n");
        ...
        for (int i = 0; i < nsubnet; ++i)
        {
            ...
            fprintf(fp, "Broadcast %d --> %d.%d.%d.%d\n ", i + 1, ip1, ip2, ip3, ip4 - 1);
        }
        fprintf(fp, "\n");
        ip2 = 0;
        printf("\n");
        for (int i = 0; i < nsubnet; ++i)
        {
            ...
            ip4 = 254;
            ip2--;
            printf("Gateway %d --> %d.%d.%d.%d\n ", i + 1, ip1, ip2, ip3, ip4 - 2);
            bin(&sub, ip1, ip2, ip3, ip4 - 2, 1);
            fprintf(fp, "Gateway %d --> %d.%d.%d.%d\n ", i + 1, ip1, ip2, ip3, ip4 - 2);
        }
        fprintf(fp, "\n");
        break;
    case 'B':
    ...
    case 'C':
        ...
        for (int i = 0; i < nsubnet; i++)
        {
            printf("NetID %d --> %d.%d.%d.%d ", i + 1, ip1, ip2, ip3, ip4);
            bin(&sub, ip1, ip2, ip3, ip4, 1);
            fprintf(fp, "NetID %d --> %d.%d.%d.%d ", i + 1, ip1, ip2, ip3, ip4);
        ...
        {
            ip4 += block;
            printf("Gateway %d --> %d.%d.%d.%d ", i + 1, ip1, ip2, ip3, ip4 - 2);
            bin(&sub, ip1, ip2, ip3, ip4 - 2, 1);
            fprintf(fp, "Gateway %d --> %d.%d.%d.%d ", i + 1, ip1, ip2, ip3, ip4 - 2);
            printf(" - %d.%d.%d.%d\n", ip1, ip2, ip3, ip4 - 3);
            bin(&sub, ip1, ip2, ip3, ip4 - 3, 1);
            fprintf(fp, " - %d.%d.%d.%d\n ", ip1, ip2, ip3, ip4 - 3);
        }
        fprintf(fp, "\n");
        break;
...
void id_sub(char classe_ip, int nvlsm, int *host, int somma, int ip3, int ip4, int ip2, int ip1)
{
    ...
    case 'A':
        if (somma < 16777216)
        {
            ...
            for (int i = 0; i < nvlsm; i++)
            {
                if ((ip4 + host[i]) >= 256)
                {
                    ...
                    if (ip4 == 0)
                    {
                        printf("gateway %d --> %d.%d.%d.%d\n ", i + 1, ip1, ip2, ip3, 255 - 1);
                        bin(&sub, ip1, ip2, ip3, 255 - 1, 1);
                        fprintf(fp, "gateway %d --> %d.%d.%d.%d\n ", i + 1, ip1, ip2, ip3, 255 - 1);
                    }
                }
                else
        ...
        {
            printf("Cannot meet request\n");
            printf("Hosts entered are %d, max 16777216 hosts for class A\n", somma);
        }
        break;
    case 'B':
        fprintf(fp, "\n");
        ...
        {
            printf("Cannot meet request\n");
            printf("Hosts entered are %d, max 65536 hosts for class B\n", somma);
        }
        break;
        fprintf(fp, "\n");
    case 'C':
        ...
        {
            printf("Cannot meet request\n");
            printf("Hosts entered are %d, max 255 hosts for class C\n", somma);
        }
        break;
    }
    fclose(fp);
...
void cleanfile()
{
    fp = fopen("ip.txt", "w");
    fclose(fp);
    printf("The file has been cleaned\n");
}
